...
<script type="text/javascript">
    ...
    $.ajax({
      ...
      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
        ...
        for (i=0; i < rdata.list.length; i++) {
          ...
          // alert(row.replyno);
          var reply = new Reply(row.memberno, row.rdate, row.replyno, row.id, row.content, row.contentsno);
          global_rdata.push(reply);
        }
  ...
  function list_by_contentsno_join_add_pg() {
    ...
    for (var i=start; i < end; i++) {
      ...
      if (global_rdata.length < global_rdata_cnt ) { // 1page = 10개 레코드, 11개인 경우 중간 종료
        break;
      }
